...
from alert_agent.infra.blob.llm_results import (
    ensure_llm_results_storage_sync,
    list_llm_folder_download_urls,
)
...
sre_agent = build_sre_analysis_agent()


@asynccontextmanager
async def lifespan(app: FastAPI):
    ensure_llm_results_storage_sync()
    await sre_agent.start()
    try:
        yield
    finally:
        await sre_agent.stop()


def create_app() -> FastAPI:
    return FastAPI(title="Grafana Alert Agent", lifespan=lifespan)
...
async def receive_alert(request: Request, background_tasks: BackgroundTasks):
    ...
    background_tasks.add_task(sre_agent.handle_and_publish, payload)
    return JSONResponse(
        status_code=202,
...
async def list_llm_result_download_urls_route():
    """
